...
def searchTwitterCounts(query, tweet_fields, user_fields, next_token, bearer_token):
    
    headers = {"Authorization": f"Bearer {bearer_token}"}
    if next_token is not None:
        url = f"https://api.twitter.com/2/tweets/counts/recent?query={query}&{tweet_fields}&{user_fields}&next_token={next_token}"
    else:
        url = f"https://api.twitter.com/2/tweets/counts/recent?query={query}&{tweet_fields}&{user_fields}"
